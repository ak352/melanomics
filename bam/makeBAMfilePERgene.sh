...
genechr=`grep -P  "\t"$genename"\t" $annotation | head -n 1 |cut -f 3  | perl -ane '$_=~s/chr//; print $_;'`
genestart=`grep -P  "\t"$genename"\t" $annotation | head -n 1 | cut -f 5`
genestop=`grep -P  "\t"$genename"\t" $annotation | head -n 1 | cut -f 6`
